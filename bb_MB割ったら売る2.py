    ...
    for i in range(start,last):
        ...
        if z>130:#過去半年と比較するため、エラー回避のためにz>130とする
            ...
            if min_band == band_width:#過去半年で今日が最小のバンド幅の時に注目
                ...
                if tomorrow_BB_B > 1 and tomorrow_MB_direction > 0 and tomorrow_band_width > band_width and tomorrow_low<(0.99*price):#注目した翌日に2σ以上になれば上昇トレンド発生と判断
                    #print('翌日上昇トレンド発生')
                    buy = 0.99*price#翌日発生したのでその一日後の終値で購入
                    count_1 = count_1 + 1
                    #print('購入額：'+str(buy))
                
                elif tomorrow_BB_B < 0 and tomorrow_MB_direction < 0 and tomorrow_band_width > band_width and tomorrow_high>(1.01*price):#注目した翌日に-2σを下回ったら下降トレンド発生と判断
                    #print('翌日下降トレンド発生')
                    decrease_buy = 1.01*price
                    count_3 = count_3 + 1
                    #print('購入額：'+str(decrease_buy))
                
                elif dat_BB_B > 1 and dat_MB_direction > 0 and dat_band_width > band_width and dat_low<(0.99*price):#2日後も同様
                    #print('2日後に上昇トレンド発生')
                    buy = (0.99*price) #2日後にトレンド発生したのでその翌日の終値で購入
                    count_1 = count_1 + 1
                    #print('購入額：'+str(buy))
                           
                elif dat_BB_B < 0 and dat_MB_direction < 0 and dat_band_width > band_width and dat_high>(1.01*price):#2日後も同様
                    #print('2日後に下降トレンド発生')
                    decrease_buy = 1.01*price
                    count_3 = count_3 + 1
                    #print('購入額：'+str(decrease_buy))
                    
                elif d3l_BB_B > 1 and d3l_MB_direction > 0 and d3l_band_width > band_width and d3l_low<(0.99*price):                   
                    buy = 0.99*price #2日後にトレンド発生したのでその翌日の終値で購
                    count_1 = count_1 + 1
                    #print('購入額：'+str(buy))
                           
                elif d3l_BB_B < 0 and d3l_MB_direction < 0 and d3l_band_width > band_width and d3l_high>(1.01*price):#2日後も同様
                    #print('2日後に下降トレンド発生')
                    decrease_buy = 1.01*price
                    count_3 = count_3 + 1
                    #print('購入額：'+str(decrease_buy))
